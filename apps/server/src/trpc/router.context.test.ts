...
mock.module("../db", () => ({
  db: {
    selectFrom: (table: string) => query(
      table === "conversation"
        ? ownedConversation ? { id: "conversation" } : undefined
...
const { DEFAULT_CONTEXT_GLOBAL_SETTINGS, parseContextGlobalSettings } = await import("../context/settings");

describe("context management metadata", () => {
  test("uses built-in settings for absent, malformed, and unsupported metadata", () => {
    expect(parseContextGlobalSettings(null)).toEqual(DEFAULT_CONTEXT_GLOBAL_SETTINGS);
    expect(parseContextGlobalSettings("not json")).toEqual(DEFAULT_CONTEXT_GLOBAL_SETTINGS);
    expect(parseContextGlobalSettings(JSON.stringify({ version: 2 }))).toEqual(DEFAULT_CONTEXT_GLOBAL_SETTINGS);
  });
  ...
  test("accepts a complete versioned policy with a prompt override", () => {
    ...
    expect(parseContextGlobalSettings(JSON.stringify(settings))).toEqual(settings);
  });
  ...
  test("rejects an empty prompt override", () => {
    ...
    expect(parseContextGlobalSettings(JSON.stringify(settings))).toEqual(DEFAULT_CONTEXT_GLOBAL_SETTINGS);
  });
  ...
  test("requires ordered policy limits and selectors matching their scope", () => {
    ...
    expect(contextPolicySchema.safeParse({ ...policy, scope: "exact_model", modelId: null }).success).toBe(false);
    expect(contextPolicySchema.safeParse({ ...policy, targetTokens: 300_000 }).success).toBe(false);
  });
  ...
  test("returns owner-visible context status without summary content", async () => {
    ...
    await expect(caller.conversation.contextState({ conversationId: "conversation" })).resolves.toEqual({
      state: "failed",
  ...
  test("rejects context status for a conversation the user does not own", async () => {
    ...
    await expect(caller.conversation.contextState({ conversationId: "conversation" })).rejects.toMatchObject({ code: "NOT_FOUND" });
