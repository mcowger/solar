...
const OVERFLOW_PATTERN =
	/prompt is too long|request_too_large|input is too long for requested model|exceeds (?:the )?(?:context window|(?:model'?s )?maximum context length|the maximum allowed input length)|input token count.*exceeds the maximum|maximum prompt length is \d+|reduce the length of the messages|maximum context length is \d+ tokens|exceeds the available context size|greater than the context length|context[_ ]length[_ ]exceeded|token limit exceeded/i;
const NON_OVERFLOW_PATTERN =
	/rate limit|too many requests|throttling error|service unavailable/i;
...
function isContextOverflow(
	...
	const errorMessage =
		message?.errorMessage ?? (error instanceof Error ? error.message : "");
	if (
		errorMessage &&
		!NON_OVERFLOW_PATTERN.test(errorMessage) &&
		OVERFLOW_PATTERN.test(errorMessage)
	)
		return true;
	if (!message || !contextWindow) return false;
	const inputTokens = message.usage.input + message.usage.cacheRead;
	return (
		(message.stopReason === "stop" && inputTokens > contextWindow) ||
