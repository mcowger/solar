...
export function buildRollingSummaryPrompt(
	...
	const prior = previous
		? renderStructuredSummary(previous)
		: "No previous summary exists.";
	const transcript = records
		.map(
			(record) =>
				`${record.role}: ${record.content.map((part) => part.text).join("\n")}`,
		)
		.join("\n\n");
	return [
...
function transactionGroups(
	...
	for (const record of records) {
		...
		if (
			...
			previous?.[0]?.toolTransactionId === record.toolTransactionId
		)
...
export function planSummaryChunks(
	records: readonly ContextRecord[],
	taskModelInputLimit: number,
	reserveTokens: number,
	estimate?: TokenEstimator,
): SummaryChunkPlan {
	const limit = taskModelInputLimit - reserveTokens;
	if (limit <= 0)
		throw new RangeError("task model limit must exceed reserveTokens");
	...
	for (const group of transactionGroups(records)) {
		...
		if (groupTokens > limit) {
			if (group[0]?.toolTransactionId)
				oversizedTransactionIds.push(group[0].toolTransactionId);
			continue;
		}
