...
describe("context assembly", () => {
	...
	test("keeps typed first-turn text and omits, rather than truncates, oversized attachments", () => {
		const attachment = {
			...
			text: "abcdefgh",
			tokenCount: 2,
			summary: '<attachment name="file-1">durable text</attachment>',
		};
		const result = assembleContext(
			[
				record("first", "user", "typed text", {
					content: [{ kind: "text", text: "typed text" }, attachment],
				}),
				record("current", "user", "current"),
			],
			{ inputLimit: 100, firstTurnAttachmentTokens: 1 },
		);
		expect(result.records.find((item) => item.id === "first")?.content).toEqual(
			[{ kind: "text", text: "typed text" }],
		);
		expect(result.omittedAttachments).toEqual([attachment]);
		expect(result.compactionRecords).toContainEqual(
			expect.objectContaining({
				id: "first:omitted-attachments",
				content: [
					expect.objectContaining({
						id: "file-1",
						kind: "text",
	...
	test("uses durable metadata when an omitted image has no textual representation", () => {
		...
		expect(result.compactionRecords[0]?.content[0]?.text).toBe(
			"[Omitted attachment: diagram.png; type: image/png; kind: image; bytes: 42]",
		);
	});
	...
	test("always retains the full current turn and its tool transaction even when it exceeds budget", () => {
		const result = assembleContext(
			[
				...
				record("current", "user", "current"),
				record("call", "assistant", "call", {
					status: "complete",
					toolTransactionId: "tx",
					content: [
						{ kind: "reasoning", text: "reasoning" },
						{ kind: "toolCall", text: "call" },
					],
				}),
				record("result", "tool", "result", {
					status: "complete",
					toolTransactionId: "tx",
					content: [{ kind: "toolResult", text: "tool output" }],
				}),
			],
			{ inputLimit: 2 },
		);
		...
		expect(
			result.records.find((item) => item.id === "call")?.content,
		).toContainEqual({ kind: "reasoning", text: "reasoning" });
		expect(
			result.records.find((item) => item.id === "result")?.content,
		).toEqual([{ kind: "toolResult", text: "tool output" }]);
		expect(result.overBudget).toBe(true);
	});

	test("omits old completed reasoning but compacts whole old tool transactions", () => {
		const oldCall = record("old-call", "assistant", "call", {
			status: "complete",
			toolTransactionId: "old-tx",
			content: [
				{ kind: "reasoning", text: "discarded" },
				{ kind: "toolCall", text: "call" },
			],
		});
		const oldResult = record("old-result", "tool", "result", {
			status: "complete",
			toolTransactionId: "old-tx",
			content: [{ kind: "toolResult", text: "result" }],
		});
		const result = assembleContext(
			[
				record("first", "user", "first"),
		...
		expect(result.compactionRecords.map((item) => item.id)).toEqual([
			"old-call",
			"old-result",
		]);
		expect(result.compactionRecords[0]?.content).toEqual([
			{ kind: "toolCall", text: "call" },
		]);
		expect(result.compactionRecords[1]?.content).toEqual([
			{ kind: "toolResult", text: "result" },
		]);
	});
...
describe("compaction", () => {
	test("renders every durable summary section and plans only safe atomic chunks", () => {
		const summary = {
			...
			constraints: ["No DB"],
			decisions: ["Use Bun"],
			durableFacts: ["Server is Hono"],
			progress: ["Tests added"],
			unresolvedQuestions: ["Integrate"],
			criticalExcerpts: ["Exact API"],
			toolOutcomes: ["Tool passed"],
		};
		...
		expect(
			buildRollingSummaryPrompt(summary, [record("u", "user", "hello")]),
		).toContain("New conversation material:");
		const records = [
			record("one", "user", "12345678"),
			record("two", "assistant", "12345678901234567890", {
				toolTransactionId: "tx",
			}),
			record("three", "tool", "12345678901234567890", {
				toolTransactionId: "tx",
			}),
		];
		...
		expect(
			plan.chunks.every((chunk) => estimateRecordsTokens(chunk) <= 8),
		).toBe(true);
	});
});
