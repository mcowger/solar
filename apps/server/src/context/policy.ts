...
export function contextPolicyFor(model: Pick<ModelContextCapabilities, "modelId" | "provider">): ContextPolicy | undefined {
  const identity = `${model.provider ?? ""} ${model.modelId ?? ""}`.toLowerCase();
  if (identity.includes("claude") || identity.includes("anthropic")) return CLAUDE_CONTEXT_POLICY;
  if (identity.includes("gpt") || identity.includes("openai")) return GPT_CONTEXT_POLICY;
  return undefined;
}
...
export function effectiveContextPolicy(
  ...
  const hardInputTokens = Math.min(configured?.hardInputTokens ?? availableInputTokens, availableInputTokens);
  return {
    enabled: configured?.enabled ?? true,
    softTriggerTokens: configured?.softTriggerTokens ?? Math.floor(model.contextWindow * 0.7),
    targetTokens: Math.min(configured?.targetTokens ?? Math.floor(model.contextWindow * 0.45), hardInputTokens),
    hardInputTokens,
    maxPinnedAttachmentTokens: Math.min(configured?.maxPinnedAttachmentTokens ?? 64_000, hardInputTokens),
    outputReserveTokens,
  };
}
