...
export async function up(db: Kysely<unknown>): Promise<void> {
  await db.schema
    ...
    .execute();
  await db.schema.createIndex("context_policy_resolution_idx").on("context_policy").columns(["scope", "provider", "modelFamily", "modelId"]).execute();

  await db.schema
    .createTable("conversation_context_state")
    .addColumn("conversationId", "text", (col) => col.primaryKey().references("conversation.id").onDelete("cascade"))
    .addColumn("revision", "integer", (col) => col.notNull().defaultTo(0))
    .addColumn("summary", "text")
    .addColumn("summaryRevision", "integer")
    .addColumn("retainedMessageBoundaryId", "text", (col) => col.references("message.id").onDelete("set null"))
  ...
  await db.schema
    ...
    .addColumn("messageId", "text", (col) => col.notNull().references("message.id").onDelete("cascade"))
  ...
  await db.schema
    ...
    .addColumn("id", "text", (col) => col.primaryKey())
    .addColumn("conversationId", "text", (col) => col.references("conversation.id").onDelete("cascade"))
    .addColumn("messageId", "text", (col) => col.references("message.id").onDelete("set null"))
    .addColumn("provider", "text", (col) => col.notNull())
    ...
    .addColumn("createdAt", "text", (col) => col.notNull().defaultTo(sql`CURRENT_TIMESTAMP`))
    .execute();
  await db.schema.createIndex("provider_call_telemetry_conversationId_idx").on("provider_call_telemetry").column("conversationId").execute();
}
