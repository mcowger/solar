...
export async function up(db: Kysely<unknown>): Promise<void> {
	...
	await db.schema
		...
		.addColumn("defaultEndpointId", "text")
		.execute();
	...
	await sql`UPDATE conversation SET endpointId = modelApi WHERE endpointId IS NULL AND modelApi IS NOT NULL`.execute(
		db,
	);
	await sql`UPDATE preset SET endpointId = modelApi WHERE endpointId IS NULL`.execute(
		db,
	);
	await sql`UPDATE user_setting SET defaultEndpointId = defaultApi WHERE defaultEndpointId IS NULL AND defaultApi IS NOT NULL`.execute(
		db,
	);
}
