						...
						return {
							executeTakeFirst: async () => {
								if (table === "provider_config" && column === "provider") {
									return state.providerConfigs.find(
...
describe("catalog model policy", () => {
	test("enables text extraction for configured models without a native attachment adapter", async () => {
		...
		await expect(
			catalog.documentInputCapabilities({
				provider: "other",
				endpointId: "openai-completions",
				modelId: "text-model",
				api: "openai-completions",
			...
			enabledModels: [
				{
					...
					endpointId: publicModel.endpointId,
					api: publicModel.api,
					visibility: "public",
				},
				{
	...
	test("uses the first still-available selection in conversation, user, admin order", async () => {
		configureModels(
			...
			},
			{
				...
				enabledModels: [
					{
						id: privateModel.modelId,
						endpointId: privateModel.endpointId,
						api: privateModel.api,
						visibility: "private",
					},
				],
			},
		...
		expect(await catalog.resolveSelection(privateModel, "user-1")).toEqual(
			publicModel,
		);
		expect(await catalog.resolveSelection(publicModel, "user-1")).toEqual(
			publicModel,
		);
	...
	test("uses only public task models and falls back when the configured task model is unavailable", async () => {
		...
		await expect(
			catalog.resolveTaskModelOrFallback(privateModel),
		).resolves.toEqual(publicModel);

		state.appMeta.set("task_model", JSON.stringify(privateModel));
		await expect(catalog.resolveTaskModel()).rejects.toThrow(
			"No task model is configured",
		);
		await expect(
			catalog.resolveTaskModelOrFallback(privateModel),
		).resolves.toEqual(privateModel);
	});
