...
describe("GenerationManager SSE lifecycle", () => {
  ...
  test("persists native tool-loop steps and every provider call while aggregating usage", async () => {
    streamFactory = async function* (_context, _selection, _params, _signal, _tools, options) {
      options.onGenerationStep({ role: "assistant", content: [{ type: "toolCall", id: "call-1", name: "weather", arguments: {} }] });
      options.onGenerationStep({ role: "toolResult", toolCallId: "call-1", toolName: "weather", content: [{ type: "text", text: "sunny" }], isError: false });
       options.onProviderCall({ purpose: "chat", observation: { provider: "test", api: "test", modelId: "model", inputTokens: 3, outputTokens: 5, cacheReadTokens: 2, cacheWriteTokens: 1, estimatedCostMicros: 4, latencyMs: 10 } });
       options.onProviderCall({ purpose: "tool_loop", observation: { provider: "test", api: "test", modelId: "model", inputTokens: 7, outputTokens: 11, latencyMs: 12 } });
      yield { ...doneEvent, message: { ...doneEvent.message, api: "test", provider: "test", model: "model" } };
    };
    const manager = new GenerationManager();
    ...
    expect(generationSteps).toEqual([{ messageId: "message-1", steps: [
      expect.objectContaining({ role: "assistant" }),
      expect.objectContaining({ role: "toolResult" }),
    ] }]);
    expect(providerCalls).toEqual([
      expect.objectContaining({ purpose: "chat", inputTokens: 3, cacheReadTokens: 2, cacheWriteTokens: 1, estimatedCostMicros: 4 }),
       expect.objectContaining({ purpose: "tool_loop", inputTokens: 7, latencyMs: 12 }),
    ]);
    expect(updates).toEqual(expect.arrayContaining([
      expect.objectContaining({ table: "message", values: expect.objectContaining({ inputTokens: 10, outputTokens: 16 }) }),
    ]));
  });
  ...
  test("records title calls with their originating conversation and message", async () => {
    titleFactory = async (_prompt, _selection, options) => {
       options.onProviderCall({ purpose: "chat", observation: { provider: "title-provider", api: "test", modelId: "title-model", inputTokens: 2, outputTokens: 3, latencyMs: 4 } });
    ...
    manager.start({
      ...
      selection: { provider: "test", endpointId: "test", modelId: "model", api: "test" },
      params: {} as never,
      titleGeneration: { firstMessage: "Hello", prompt: "{{first_message}}", selection: { provider: "title-provider", endpointId: "test", modelId: "title-model", api: "test" } },
    });
    await readEvents(manager.subscribe("message-1"));

    expect(providerCalls).toEqual(expect.arrayContaining([
      expect.objectContaining({ purpose: "title", conversationId: "conversation-1", messageId: "message-1", inputTokens: 2 }),
    ]));
  });

  test("persists overflow telemetry when a provider call fails", async () => {
    streamFactory = async function* (_context, _selection, _params, _signal, _tools, options) {
       options.onProviderCall({ purpose: "chat", observation: { provider: "test", api: "test", modelId: "model", inputTokens: 9, outputTokens: 0, cacheReadTokens: 4, cacheWriteTokens: 2, latencyMs: 10, overflowed: true, error: { kind: "context_overflow", retrySafe: true, outputStarted: false, toolStepsCompleted: false } } });
    ...
    expect(providerCalls).toEqual([expect.objectContaining({ overflowed: true, cacheReadTokens: 4, cacheWriteTokens: 2 })]);
    expect(providerCalls[0]).not.toHaveProperty("error");
    expect(updates).toEqual(expect.arrayContaining([
      expect.objectContaining({ table: "message", values: expect.objectContaining({ inputTokens: 9, outputTokens: 0, status: "error" }) }),
    ]));
  });
  ...
  test("compacts and retries exactly once after safe overflow before output", async () => {
    ...
    streamFactory = async function* (_context, _selection, _params, _signal, _tools, options) {
      calls++;
      if (calls === 1) {
         options.onProviderCall({ purpose: "chat", observation: { provider: "test", api: "test", modelId: "model", overflowed: true, retryAttempt: 0, error: { kind: "context_overflow", retrySafe: true, outputStarted: false, toolStepsCompleted: false } } });
        yield { type: "error", error: { errorMessage: "context exceeded" } };
        return;
      }
    ...
    const manager = new GenerationManager();
    manager.start({
      conversationId: "conversation-1", messageId: "message-1", context: {} as never,
      selection: { provider: "test", endpointId: "test", modelId: "model", api: "test" }, params: {} as never,
      retryContext: async () => { rebuilt++; return { context: { messages: [] }, params: {} as never }; },
    });
    expect(await readEvents(manager.subscribe("message-1"))).toEqual(expect.arrayContaining([
      expect.objectContaining({ data: { type: "text-delta", textDelta: "Recovered" } }),
    ]));
  ...
  test("does not retry overflow after streamed output", async () => {
    let rebuilt = 0;
    streamFactory = async function* (_context, _selection, _params, _signal, _tools, options) {
       options.onProviderCall({ purpose: "chat", observation: { provider: "test", api: "test", modelId: "model", overflowed: true, error: { kind: "context_overflow", retrySafe: false, outputStarted: true, toolStepsCompleted: false } } });
      yield { type: "text_delta", delta: "Partial" };
      yield { type: "error", error: { errorMessage: "context exceeded" } };
    };
    const manager = new GenerationManager();
