...
describe("piEventToUiChunks", () => {
	test("maps text, reasoning, result, completion, and error events", () => {
		expect(
			piEventToUiChunks(
				{ type: "text_delta", delta: "Hello" } as AssistantMessageEvent,
				toolDisplayNames,
			),
		).toEqual([{ type: "text-delta", textDelta: "Hello" }]);
		expect(
			piEventToUiChunks(
				{ type: "thinking_delta", delta: "Consider" } as AssistantMessageEvent,
				toolDisplayNames,
			),
		).toEqual([{ type: "reasoning-delta", delta: "Consider" }]);
		expect(
			piEventToUiChunks(
				{
		...
		expect(
			piEventToUiChunks(
				{ type: "error", error: {} } as AssistantMessageEvent,
				toolDisplayNames,
			),
		).toEqual([{ type: "error", errorText: "error" }]);
	});

	test("maps tool lifecycle events using the resolved tool metadata", () => {
		const partial = {
			content: [{ type: "toolCall", id: "call-1", name: "mcp_weather" }],
		};
		expect(
			piEventToUiChunks(
				{
		...
		).toEqual([{ type: "tool-call-end", toolCallId: "call-1" }]);
	});
