...
function injectOpenAIResponsesDocuments(payload: unknown, documents: NativeDocumentInput[]) {
  ...
  for (const message of input as { content?: unknown }[]) {
    ...
    message.content = message.content.flatMap((part) => {
      ...
      const document = text.type === "input_text" && typeof text.text === "string"
...
function injectAnthropicDocuments(payload: unknown, documents: NativeDocumentInput[]) {
  ...
  for (const message of messages as { content?: unknown }[]) {
    ...
    message.content = message.content.flatMap((part) => {
      const text = part as { type?: unknown; text?: unknown };
      const document = text.type === "text" && typeof text.text === "string"
        ? byMarker.get(text.text)
        : undefined;
