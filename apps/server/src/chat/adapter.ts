 ...
 */
export function piEventToUiChunks(
	event: AssistantMessageEvent | ToolCallResultEvent,
	toolDisplayNames: Map<string, { serverName: string; remoteName: string }>,
): UiChunk[] {
	...
	switch (event.type) {
		...
		case "error":
			return [
				{ type: "error", errorText: event.error.errorMessage ?? "error" },
			];
		default:
			return [];
	}
