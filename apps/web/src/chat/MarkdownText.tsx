...
function SourceFavicon({ citation }: { citation: Citation }) {
  return (
    <span className="solar-source-favicon" title={citation.domain}>
      ...
      <img src={citation.favicon} alt="" onError={(event) => { event.currentTarget.hidden = true; }} />
    </span>
  );
}
      ...
      <summary>
        <span className="solar-source-favicon-stack">
          {visibleCitations.map((citation, index) => (
            <SourceFavicon key={`${citation.url}-${index}`} citation={citation} />
          ))}
          {hiddenCitationCount > 0 && <span className="solar-source-overflow">+{hiddenCitationCount}</span>}
        </span>
  ...
  return (
    <>
      <MarkdownTextPrimitive
        ...
        preprocess={(value) => rewriteLatexBracketDelimiters(removeCitationBlocks(value))}
