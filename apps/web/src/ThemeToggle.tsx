...
const THEMES = [
  "solar-light", "solar-dark", "light", "dark", "cupcake", "bumblebee",
  "emerald", "corporate", "synthwave", "retro", "cyberpunk", "valentine",
      ...
      <dialog ref={dialogRef} className="modal modal-bottom sm:modal-middle">
        <div className="modal-box max-h-[85dvh] w-full max-w-2xl overflow-hidden p-0">
          <div className="flex items-center justify-between border-b border-base-300 px-5 py-4">
            <div>
              <h2 className="font-sans text-lg font-semibold">Choose a theme</h2>
              <p className="text-sm opacity-60">{THEMES.length} themes, saved automatically</p>
            </div>
            <button type="button" className="btn btn-ghost btn-sm btn-circle" onClick={() => dialogRef.current?.close()}>
              <X size={18} />
            </button>
          </div>
            ...
            <div className="grid max-h-[55dvh] grid-cols-2 gap-2 overflow-y-auto pr-1 sm:grid-cols-3">
              {visibleThemes.map((theme) => (
                ...
                >
                  ...
                  {currentTheme === theme && <Check className="shrink-0" size={16} />}
                </button>
              ))}
            </div>
          </div>
