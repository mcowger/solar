...
const signInEmail = mock(
  async (_credentials: { email: string; password: string }): Promise<AuthResult> => ({ error: null }),
);
const signUpEmail = mock(
  async (_credentials: { email: string; password: string; name: string }): Promise<AuthResult> => ({ error: null }),
);
...
describe("AuthForm", () => {
  test("submits email and password when signing in", async () => {
    ...
    await user.type(screen.getByPlaceholderText("Password (min 8)"), "password");
  ...
  test("switches to registration and displays an authentication error", async () => {
    signUpEmail.mockResolvedValueOnce({ error: { message: "Email is already registered" } });
    const user = userEvent.setup();
    render(<AuthForm />);
