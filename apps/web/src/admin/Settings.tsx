				...
				<fieldset className="fieldset gap-2">
					...
					{endpoints.map((endpoint) => (
						<div key={endpoint.id} className="rounded-box bg-base-200 p-2">
							<div className="grid gap-2 sm:grid-cols-[10rem_minmax(0,1fr)_11rem_auto]">
								<input
								...
								>
									{endpoints.map((endpoint) => (
										<option key={endpoint.id} value={endpoint.id}>
											{endpoint.label || endpoint.api} · {endpoint.api}
										</option>
									))}
								</select>
								<div className="flex min-w-0 flex-wrap items-center gap-2 sm:col-span-2">
									...
									<label className="flex items-center gap-1.5 whitespace-nowrap text-sm">
										<input
											...
											onChange={(event) =>
												updateModel(index, { documents: event.target.checked })
											}
										/>
										<span>Documents</span>
									</label>
										...
										<select
											...
											onChange={(event) =>
												updateModel(index, {
													reasoningEffort:
														(event.target
															.value as AllowlistEntry["reasoningEffort"]) ||
														undefined,
												})
											}
										>
										...
										<select
											...
											onChange={(event) =>
												updateModel(index, {
													verbosity:
														(event.target
															.value as AllowlistEntry["verbosity"]) ||
														undefined,
												})
											}
										>
