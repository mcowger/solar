...
const userEmail = values.user ?? process.env.SOLAR_LOCAL_USER_EMAIL ?? DEFAULT_USER_EMAIL;
const users = await trpcRequest(baseUrl, cookie, "admin.listUsers", undefined, "GET") as { id: string; email: string }[];
const user = users.find((candidate) => candidate.email.toLowerCase() === userEmail.toLowerCase());
...
const summary = await trpcRequest(baseUrl, cookie, "admin.history.import", { userId: user.id, history }, "POST");
