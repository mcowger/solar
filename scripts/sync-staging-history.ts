...
function sessionCookieFromHeaders(headers: Headers) {
  ...
  const cookie = (getSetCookie ? getSetCookie.call(headers) : [headers.get("set-cookie") ?? ""])
...
async function getSessionCookie(baseUrl: string, cookie: string | undefined, email: string | undefined, password: string | undefined) {
  ...
  if (!email || !password) fail("Provide a session cookie, or both an admin email and password.");
  const response = await fetch(`${baseUrl}/api/auth/sign-in/email`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ email, password }),
  });
  if (!response.ok) fail(`Admin login failed (${response.status}): ${await response.text()}`);
  return sessionCookieFromHeaders(response.headers);
}

async function trpcRequest(baseUrl: string, cookie: string, path: string, input: unknown, method: "GET" | "POST") {
  const requestUrl = new URL(`/trpc/${path}`, baseUrl);
  const headers = new Headers({ cookie });
  let body: string | undefined;
  if (method === "GET") requestUrl.searchParams.set("input", JSON.stringify(input));
  else {
  ...
  if (!response.ok || payload.error) fail(`tRPC ${path} failed: ${payload.error?.json?.message ?? response.statusText}`);
  const data = payload.result?.data;
  if (data === undefined) fail(`tRPC ${path} returned no data.`);
  return typeof data === "object" && data !== null && "json" in data ? (data as { json: unknown }).json : data;
}

async function userIdForEmail(baseUrl: string, cookie: string, email: string) {
  const users = await trpcRequest(baseUrl, cookie, "admin.listUsers", undefined, "GET") as User[];
...
const stagingUrl = (values["staging-url"] ?? process.env.SOLAR_STAGING_URL ?? DEFAULT_STAGING_URL).replace(/\/$/, "");
const localUrl = (values["local-url"] ?? process.env.SOLAR_URL ?? DEFAULT_LOCAL_URL).replace(/\/$/, "");
const stagingCookie = await getSessionCookie(
  stagingUrl,
  values["staging-cookie"] ?? process.env.SOLAR_STAGING_SESSION_COOKIE,
  values["staging-email"] ?? process.env.SOLAR_STAGING_ADMIN_EMAIL ?? "devuser@cowger.us",
  values["staging-password"] ?? process.env.SOLAR_STAGING_ADMIN_PASSWORD ?? "password",
);
const localCookie = await getSessionCookie(
  localUrl,
  values["local-cookie"] ?? process.env.SOLAR_SESSION_COOKIE,
  values["local-email"] ?? process.env.SOLAR_ADMIN_EMAIL ?? DEFAULT_LOCAL_USER_EMAIL,
  values["local-password"] ?? process.env.SOLAR_ADMIN_PASSWORD ?? "password",
);
const stagingUserId = await userIdForEmail(stagingUrl, stagingCookie, values["staging-user"] ?? process.env.SOLAR_STAGING_USER_EMAIL ?? DEFAULT_STAGING_USER_EMAIL);
const localUserId = await userIdForEmail(localUrl, localCookie, values["local-user"] ?? process.env.SOLAR_LOCAL_USER_EMAIL ?? DEFAULT_LOCAL_USER_EMAIL);
const history = await trpcRequest(stagingUrl, stagingCookie, "admin.history.export", { userId: stagingUserId }, "GET");
const output = values.output ?? DEFAULT_OUTPUT;
await Bun.write(output, `${JSON.stringify(history, null, 2)}\n`);
const summary = await trpcRequest(localUrl, localCookie, "admin.history.import", { userId: localUserId, history }, "POST");
console.log(`Exported staging history to ${output} and restored it for ${values["local-user"] ?? process.env.SOLAR_LOCAL_USER_EMAIL ?? DEFAULT_LOCAL_USER_EMAIL}.`);
